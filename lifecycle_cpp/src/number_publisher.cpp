...
public:
    NumberPublisherNode() : LifecycleNode("number_publisher")
    {
        ...
        RCLCPP_INFO(get_logger(), "In unconfigured state");
    }
    
    LifecycleCallbackReturn on_configure(const rclcpp_lifecycle::State &state)
    {
        (void)state;
        RCLCPP_INFO(get_logger(), "In on_configure state");
    ...
    {
        (void)state;
        RCLCPP_INFO(get_logger(), "In on_cleanup state");
    ...
    {
        RCLCPP_INFO(get_logger(), "In on_activate state");
        number_timer_->reset();
    ...
    {
        RCLCPP_INFO(get_logger(), "In on_deactivate state");
        number_timer_->cancel();
    ...
    {
        RCLCPP_INFO(get_logger(), "In on_shutdown state");
        if (number_publisher_)
        {
            number_publisher_.reset();
        }
        if (number_timer_)
    ...
    {
        RCLCPP_INFO(get_logger(), "In on_error state");
