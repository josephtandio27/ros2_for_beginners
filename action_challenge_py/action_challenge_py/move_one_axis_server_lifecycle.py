...
class MoveOneAxisServerNode(LifecycleNode):
    def __init__(self):
        super().__init__("move_one_axis_server")
        self.action_name_ = None
        self.current_position_ = None
        self.goal_handle_ : ServerGoalHandle = None
        self.goal_lock_ = None
        self.move_server_ = None
        self.node_active_ : bool = None
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.declare_parameter("action_name", "move_one_axis")
        self.action_name_ = self.get_parameter("action_name").value
        self.current_position_ = 50
        self.goal_handle_ : ServerGoalHandle = None
        self.node_active_ = False
    ...
    def on_cleanup(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.action_name_ = None
        if self.move_server_ is not None:
            self.move_server_.destroy()
            self.move_server_ = None
        if self.goal_lock_ is not None:
            del self.goal_lock_
            self.goal_lock_ = None
        if self.goal_handle_ is not None:
            del self.goal_handle_
            self.goal_handle_ = None
        return super().on_cleanup(state)

    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        self.get_logger().info("In on_activate state")
        self.node_active_ = True
    ...
    def goal_callback(self, goal_request: MoveOneAxis.Goal):
        ...
        # Check if node is on active state
        if not self.node_active_:
            self.get_logger().warn("Goal request rejected. Node is not active")
            return GoalResponse.REJECT
        
        # Check the goal request
        if goal_request.target_position < 0 or goal_request.target_position > 100:
            self.get_logger().warn("Goal request rejected. Target position must be between 0 and 100")
            return GoalResponse.REJECT
        if goal_request.velocity < 0:
            self.get_logger().warn("Goal request rejected. Velocity must be positive")
            return GoalResponse.REJECT
